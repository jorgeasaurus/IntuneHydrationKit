function Write-HydrationExecutionSummary {
    ...
    function Get-MobileAppExampleScriptsPath {
        param(
            [Parameter(Mandatory)]
            [hashtable]$ExecutionSettings
        )

        if ($ExecutionSettings.imports.mobileApps -ne $true) {
            return $null
        }
        ...
        $referencePath = Join-Path -Path $moduleRoot -ChildPath 'Reference'
    ...
    $mobileAppExampleScriptsPath = Get-MobileAppExampleScriptsPath -ExecutionSettings $Settings
...
"@
    ...
    if ($script:GeneratedScriptsPath) {
        $reportContent += @"

## Generated Scripts

- Generated PowerShell scripts were saved to: $($script:GeneratedScriptsPath)

"@
    }

    if ($mobileAppExampleScriptsPath) {
        $reportContent += @"

## Mobile App Script Examples

- Example WinGet wrapper and remediation scripts are available under: $mobileAppExampleScriptsPath
...
"@
    ...
    if ($script:GeneratedScriptsPath) {
        $summaryLines += Format-HydrationDisplayMessage -Message "Scripts: $($script:GeneratedScriptsPath)" -Style 'Info' -Emoji '📂'
    }

    if ($mobileAppExampleScriptsPath) {
        $summaryLines += Format-HydrationDisplayMessage -Message "Examples: $mobileAppExampleScriptsPath" -Style 'Info' -Emoji '🧩'
    }
