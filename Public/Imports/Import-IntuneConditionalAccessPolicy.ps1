function Import-IntuneConditionalAccessPolicy {
    ...
    if (-not $templateFiles -or $templateFiles.Count -eq 0) {
        Write-HydrationLog -Message "No Conditional Access templates found in: $TemplatePath" -Level Warning
        return @()
    }
    ...
    $hasPremiumP2 = $false
    try {
        $subscribedSkusResponse = Invoke-MgGraphRequest -Method GET -Uri "beta/subscribedSkus?`$select=id,capabilityStatus,servicePlans" -ErrorAction Stop
        $hasPremiumP2 = Test-HydrationHasPremiumP2License -SubscribedSkus @($subscribedSkusResponse.value) -PremiumP2ServicePlans $premiumP2ServicePlans
    } catch {
    ...
    if (-not $hasPremiumP2) {
        Write-HydrationLog -Message 'No Azure AD Premium P2 license detected. Risk-based Conditional Access policies will be skipped.' -Level Warning
    }
    ...
    function Get-ExistingConditionalAccessPolicies {
        ...
        try {
            ...
            Write-HydrationLog -Message "Could not retrieve existing CA policies: $_" -Level Warning
        }
