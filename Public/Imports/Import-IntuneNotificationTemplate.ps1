    ...
    if ($RemoveExisting) {
        ...
        foreach ($templateName in $existingTemplates.Keys) {
            ...
            if ($PSCmdlet.ShouldProcess($templateName, "Delete notification template")) {
                ...
                $results += Add-HydrationDryRunResult -Action 'WouldDelete' -Name $templateName -Type 'NotificationTemplate'
            }
        }
    ...
    foreach ($templateFile in $templateFiles) {
        try {
            ...
            if ($PSCmdlet.ShouldProcess($displayName, "Create notification template")) {
                ...
                $results += Add-HydrationDryRunResult -Action 'WouldCreate' -Name $displayName -Path $templateFile.FullName -Type 'NotificationTemplate'
            }
        } catch {
            $errMessage = Get-GraphErrorMessage -ErrorRecord $_
